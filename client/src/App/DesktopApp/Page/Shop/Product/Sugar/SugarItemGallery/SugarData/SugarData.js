...
const SugarData = ( props ) => {
    ...
    const [ sugarOne ] = useState( () => {
        return `<p>
                <span>
                    המהדורה הראשונה של הספר "שוגר" נכתבה על ידי היוצר של מותג הסקייטבורד "מיס שוגר": איתי צוקר.
                    הספר הוא אוסף שירים ואיורים שכתב איתי כדי לעשות לעצמו סדר במחשבות.
                    תוך כדי כתיבת הספר התבשל הרעיון להקמת מותג הסקייטבורד "מיס שוגר" ובסוף המסע, הבשיל הרעיון לנקודה שבא נמצא היום.
                </span>
                <p>&nbsp;</p>
                <p>דפים: 76</p>
                <p>עברית: 37</p>
